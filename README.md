...
这种架构解决了 Telegram Bot API 的局限性，使机器人能够访问并索引完整的群组历史消息。
...
1. 设置环境变量（API密钥、代理等）
2. 运行 `auth_user.py` 脚本，使用您的 Telegram 用户账号登录，获取会话文件
...
系统会要求您输入手机号和验证码登录您的 Telegram 账号。登录成功后，会生成会话文件，用于用户客户端访问历史消息。

> **重要提示**：确保您的用户账号已加入所有需要索引的群组
...
4. 两个客户端共享同一个数据库，形成完整的搜索解决方案

这种架构设计确保了机器人能够提供完整的群组媒体文件搜索功能，同时遵守Telegram的API限制。
